...
let url = "https://www.googleapis.com/customsearch/v1?key=AIzaSyDmU_BbPI7z7RQXYVc1Hs4fX8QAxC7MvyE&cx=003429913069680451282:ffpf7-envl0&q=site:amazon.com/shop";
let amazonUrls = [];
let userInfo = {};
...
function evaluateSocialNetwork(url) {
...
async function fetchInfo(page, amazonUrl) {
    try {
        ...
        await page.exposeFunction("evaluateSocialNetwork", evaluateSocialNetwork);
        let social_link_url = {};
        social_link_url = await page.evaluate(async() => {
            let social_link_url = {}

            const social_links = document.querySelectorAll('.social-link > a');
            for (const social_link of social_links) {
                const socialType = await evaluateSocialNetwork(social_link.href);
                social_link_url[socialType] = social_link.href;
            }
        ...
        if (social_link_url.facebook) {
            ...
            try {
                await page.waitForSelector('._6tb5');
        ...
        if (social_link_url.twitter) {
            ...
            try {
                await page.waitForSelector('.r-11mg6pl');
        ...
        if (social_link_url.youtube) {
            ...
            try {
                ...
                await page.waitForSelector('#channel-header-container  #avatar img', { timeout: 0 });
        ...
        await page.waitForSelector('.shop-affiliate-profile-logo-image', { timeout: 0 });
        let Categories = [];
        if (userInfo.IdeaLists > 1) {
            ...
            for (const ideaListUrl of ideaListsUrl) {
                ...
                console.log("----Get all Idea Lits Url----", ideasUrl);
                for (const ideaUrl of ideasUrl) {
                    console.log("----------------Scrap the category from Ideal List----------------");
                    ...
                    try {
                        console.log("---------------WaitforSelector ProdctTitle----------------");
                        await page.waitForSelector('#productTitle', {
                            timeout: 5000
                                // Remove the timeout
                        });
                        console.log("---------------Getting Categoreis----------------");
                        const category = await page.$$eval('#wayfinding-breadcrumbs_feature_div li', nodes => nodes.map(node => node.innerText))
                        console.log("------Join Categories--------", category);
                        let productCategory = category.join(' ');
                        console.log("-------Product Category----------", productCategory);

                        Categories = Categories.concat(productCategory);
...
async function getAllShopsInfo() {
    ...
    const connection = mysql.createConnection({
        host: "intelstor.cqb93xnsbho4.us-east-2.rds.amazonaws.com", // ip address of server running mysql
        user: "referazon_scrape", // user name to your mysql database
        password: "Amazon123", // corresponding password
        database: "referazon_scrape" // use the specified database
    });
    try {
    ...
    const userInfo = await fetchInfo(page, "https://www.amazon.com/shop/jillianmichaels");
    let amazonShops = {};
    let categoryJson = {};
